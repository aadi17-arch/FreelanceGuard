  ...
  return (
    ...
    >
      ...
      <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-[#0A0F1D] dark:bg-[#020617] text-rui-light rounded-2xl p-8 relative overflow-hidden group border border-white/5">
           <div className="relative z-10 space-y-6">
              <div className="space-y-2">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-rui-success">System Status: Active</p>
                <h1 className="text-2xl md:text-3xl font-bold !text-rui-light leading-tight">Project Overview: {user?.name}</h1>
              ...
              <div className="flex gap-3">
                 <Link to={user?.role === 'CLIENT' ? '/create-project' : '/marketplace'}>
                    <button className="px-6 py-2.5 bg-[#1D9E75] hover:bg-[#1D9E75]/90 text-rui-light rounded-lg text-[10px] font-black uppercase tracking-wider transition-all">
                      {user?.role === 'CLIENT' ? 'Post New Project' : 'Find Opportunities'}
                    </button>
                 </Link>
                 <Link to="/escrow">
                    <button className="px-6 py-2.5 bg-white/10 hover:bg-white/20 text-rui-light rounded-lg text-[10px] font-black uppercase tracking-wider transition-all">
                      View Financials
                    </button>
                 </Link>
              </div>
