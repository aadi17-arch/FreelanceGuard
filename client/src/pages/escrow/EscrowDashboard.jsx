...
export default function SafeHoldingDashboard() {
  ...
  const handleExport = () => {
        ...
        tx.type === "DEPOSIT"
          ...
          : tx.type === "WITHDRAWAL"
            ...
            : tx.type === "WALLET_ADD"
              ...
              : tx.contract?.project?.title ||
                tx.milestone?.title ||
                "Project payment";
      const amountStr = `${userType ? "+" : "-"}${tx.amount}`;
    ...
    const csv = [titles.join(","), ...rows.map((r) => r.join(","))].join("\n");
    ...
    link.setAttribute(
      "download",
      `FreelanceGuard_Ledger_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="space-y-4 pb-4">
        <div className="flex justify-end">
          <button
            ...
            className="px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-sm font-bold transition-all flex items-center gap-2"
          >
            {user?.role === "CLIENT" ? (
              <>
                <Plus size={14} className="text-sky-100" /> Add to wallet
              </>
            ) : (
              <>
                <ArrowUpRight size={14} className="text-sky-100" /> Withdraw
              </>
            )}
          </button>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-emerald-100 border border-emerald-200 rounded-xl p-6 flex flex-col justify-between">
          <p className="text-xs font-bold text-emerald-800 mb-1">
            Available balance
          </p>
          <div className="flex items-baseline gap-2 mb-4">
            <span className="text-[28px] font-bold tracking-tight text-emerald-950">
              ${(liveUser || user)?.walletBalance?.toLocaleString() || "0"}
            </span>
        ...
        <div className="bg-sky-100 border border-sky-200 rounded-xl p-6 flex flex-col justify-between">
          <p className="text-xs font-bold text-sky-800 mb-1">
            {user?.role === "CLIENT" ? "Total spent" : "Total earned"}
          </p>
          <div className="flex items-baseline gap-2 mb-4">
            <span className="text-[28px] font-bold tracking-tight text-sky-950">
              $
              {user?.role === "CLIENT"
                ? (liveUser || user)?.totalSpent?.toLocaleString() || "0"
                : (liveUser || user)?.totalEarned?.toLocaleString() || "0"}
            </span>
            <span className="text-[10px] font-bold text-sky-700">USD</span>
          </div>
        </div>

        <div className="bg-amber-100 border border-amber-200 rounded-xl p-6 flex flex-col justify-between">
          <p className="text-xs font-bold text-amber-800 mb-1">
            {user?.role === "CLIENT" ? "Held in projects" : "Locked in vault"}
          </p>
          <div className="flex items-baseline gap-2 mb-4">
            <span className="text-[28px] font-bold tracking-tight text-amber-950">
              ${(liveUser || user)?.heldAmount?.toLocaleString() || "0"}
            </span>
            <span className="text-[10px] font-bold text-amber-700">USD</span>
          </div>
        </div>
      </section>
          ...
          {transactions.length > 0 ? (
            transactions.map((tx) => {
              ...
              return (
                <div
                  key={tx.id}
                  className="px-8 py-5 flex items-center justify-between hover:bg-zinc-50 transition-all group"
                >
                  <div className="flex items-center gap-5">
                    <div
                      className={`w-10 h-10 rounded-lg flex items-center justify-center ${isIncoming ? "bg-emerald-50 text-emerald-600" : "bg-zinc-50 text-zinc-400"}`}
                    >
                      {tx.type === "DEPOSIT" ? (
                        <Plus size={16} />
                      ) : tx.type === "WITHDRAWAL" ? (
                  ...
                  <div className="flex items-center gap-10">
                    <div className="text-right hidden sm:block">
                      <p
                        className={`text-sm font-bold tracking-tight ${isIncoming ? "text-emerald-600" : "text-zinc-900"}`}
                      >
